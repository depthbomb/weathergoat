...
interface IRadarCommand extends ICommand {
	[kRadarCollection]: Collection<string, string>;
}

const kRadarCollection = Symbol('radar-collection');

export const radarCommand: IRadarCommand = ({
	...
	[kRadarCollection]: new Collection([
		// Stations
	...
	async handle(interaction) {
		...
		if (!this[kRadarCollection].find(v => v === station)) {
	...
	async handleAutocomplete(interaction) {
		...
		const filtered = this[kRadarCollection].filter((v, k) => k.toLowerCase().includes(value) || v.toLowerCase().includes(value));
