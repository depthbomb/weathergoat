...
export default class RadarCommand extends Command {
	private readonly _radarCollection: Collection<string, string>;
	...
	public constructor() {
		...
		this._radarCollection = new Collection([
			// Stations
	...
	public async handleAutocomplete(interaction: AutocompleteInteraction<CacheType>) {
		...
		const filtered = this._radarCollection.filter((v, k) => k.toLowerCase().includes(value) || v.toLowerCase().includes(value));
		const limited  = [...filtered.entries()].slice(0, 25); // Limit the results to 25

		await interaction.respond(limited.map(([k, v]) => ({ name: k, value: v })));
	}
}
