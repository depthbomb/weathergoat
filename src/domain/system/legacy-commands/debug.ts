...
export class DebugCommand extends BaseLegacyCommand {
	public constructor(private readonly features = inject(FeaturesService)) {
		super({
			...
			subcommands: {
				[Subcommands.Print]: [LegacyCommandParam.string('domain')],
				[Subcommands.DumpDb]: [],
			},
		});
	}
	...
	public async [Subcommands.DumpDb](message: Message) {
		const date = new Date();
		const [alertDestinations, forecastDestinations, autoRadarMessages, sentAlerts, volatileMessages] =
			await Promise.all([
				db.orm.public.AlertDestination.all(),
				db.orm.public.ForecastDestination.all(),
				db.orm.public.AutoRadarMessage.all(),
				db.orm.public.SentAlert.all(),
				db.orm.public.VolatileMessage.all(),
			]);
